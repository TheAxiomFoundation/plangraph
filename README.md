...
| W117 | An item that starts earlier, or fits inside the horizon only, in a scenario that only tightens another the plan carries: a substitution leveling made, with the seat the item waited for in the other scenario and the work that left it. A scenario tightens another when it levels whenever the other does, scales effort and duration at least as much, drops the same items, and hires no earlier, counting a later or dropped hire only on a role with `fallback: null` or whose first hire does not move (`tightens()`). Each scenario is compared with the nearest scenarios it tightens, one per distinct schedule (`tightenedFrom()`). |
